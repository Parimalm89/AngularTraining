    ...
    public class DapperHelper : IDapperHelper
    {
        ...
        public string ExecuteNonQuery(string query, CommandType cmdtype)
        {
            try
            {
                ...
                return intRowsAffected.ToString();
        ...
        public string ExecuteScalarValue(string query, CommandType cmdType)
        {
        ...
        public DataSet ExecuteSql(string sp, string tablenm, List<SqlParameter> param = null)
        {
            DataSet ds = new DataSet();
            try
            {
                using (SqlConnection objConnection = new SqlConnection(con.ConnectionString))
                {
                    objConnection.Open();
                    using (SqlCommand objCommand = new SqlCommand(sp, objConnection))
                    {
                        ...
                        using (SqlDataAdapter objAdpter = new SqlDataAdapter(objCommand))
                        {
                            using (DataSet objDataSet = new DataSet())
                            {
                                objAdpter.Fill(objDataSet, tablenm);
                                if (objDataSet.Tables[tablenm].Rows.Count > 0)
                                    ds = objDataSet;
            ...
            {
                return ds;
                //return "-4:" + Environment.NewLine + "<:--- Message  ---:>" + Environment.NewLine + ex.Message + Environment.NewLine + "<:--- StackTrace  ---:>" + Environment.NewLine + ex.StackTrace;
            }
        }
